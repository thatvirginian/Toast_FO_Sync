...
from Tables.Orders_Pull_Update import run_future_order_update
...
def main():
    ...
    # ── Future order pull ─────────────────────────────────────────────────────
    try:
        run_future_order_update(days_ahead=14, engine=engine, API=api)
    except Exception as e:
        logger.critical(f"Future order sync failed: {e}", exc_info=True)
        sys.exit(1)
